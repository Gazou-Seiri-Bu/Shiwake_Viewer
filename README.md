...
フォルダに削除するファイルの方が圧倒的に多い場合などは残す画像だけチェックで選んで

別のフォルダに移動したりチェックしていない画像をまとめて削除したりできます。
...
ComfyUIもワークフローのメタデータからプロンプトらしき部分を正規表現を使って抽出表示できます。
...
テキストを付けた画像をメタデータを残したまま保存できます。


## ＜実行前に一読をお願いします＞

◯exe形式なので実行前にダウンロードしたファイルのセキュリティスキャンをお願いします。私に悪意が無くても悪意のある人間にアカウントのパスワードを知られてファイルがすり替えられることもあります。

◯デフォルトでは削除したファイルは一旦削除フォルダに移動して後でまとめてごみ箱に送る設定になっています。直接ごみ箱に送るように変更出来ますがその場合ファイル削除の操作履歴は残らずこのソフトでのアンドゥはできません。

◯正常に動作しなくなった場合：仕分けViewer.exeと同じフォルダに自動的に作られる設定ファイルのconfig.txtが原因の可能性があります。一旦config.txtを別フォルダに移すなどして動作を試して下さい。手をかけてカスタムした設定のconfig.txtはコピーしておくことをおすすめします。
...
（zipでダウンロードすると動かないという報告をいただいたのでexeをそのままダウンロードしてみてください。）

「Windows によって PC が保護されました　Microsoft Defender SmartScreen は認識されないアプリの起動を停止しました。このアプリを実行すると、PC が危険にさらされる可能性があります。」

のメッセージが出ますが詳細情報ボタンから実行を選んでください。（自分でもドキドキするのでくれぐれもセキュリティスキャンをお願いします）
...
　　「APPsボタン」を右クリックでファイルを拡張子別のデフォルトアプリに送って起動
...
　　右クリックで画像の表示切替え（スクロール表示、全画面表示、枠内固定表示）
  
　　ホイール上下：音量調節

　　設定画面中のスクリーンショットボタンでテキストを付けた画像を保存できます。
...
MITライセンスで公開されているTakenoko3333さんのコードを利用してメタデータを保持したままフォルダ内のpng画像をwebp画像に一括変換してみます。

https://github.com/Takenoko3333/png2webp-for-a1111-and-NAI

（pythonをインストールしていることが前提です。）

１．このリポジトリのpng2webp_commandline.pyをダウンロードして適当なフォルダに置きます。コード内で書くようになっていた対象フォルダパスをコマンドライン引数で取るように手を加えさせて頂きました

２．仕分けViewer左下の「APPs」ボタンをクリック

３．「表示フォルダを設定したアプリケーションに送って起動します」の設定ボタンでダウンロードしたpng2webp_commandline.pyを選んでください

４．ファイル名が表示された緑色のボタンを押すと閲覧中の表示フォルダ内のpng画像がwebpに一括変換されます。表示フォルダと同じ階層に「（表示フォルダ名）-webp」というフォルダが作られて変換されたwebpがその中に置かれます。元のフォルダ内の画像はそのままです。


## ◎バグ報告にご協力をお願いします

起動後いきなり落ちるという方はお手数ですがrun_viewer.batをダウンロードして同じフォルダに置きそこから起動してみてください。

コマンドプロンプトは落ちずに残るはずですのでエラーメッセージが表示されていたら報告いただけるとありがたいです。

また、現在実行中に突然プログラムが落ちる問題を確認しています。原因はスライドショーの音楽再生と思われます。

おまけ要素が原因で作業が中断しては本末転倒ですので改善されるまでは
